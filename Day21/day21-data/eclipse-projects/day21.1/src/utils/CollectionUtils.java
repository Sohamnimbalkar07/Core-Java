...
public interface CollectionUtils {
	...
	//add a static method to sort student details from the map , as per GPA , ret sorted strm
	static Stream<Student> sortStudentsGPA(Map<String, Student> map)
	{
		return map.values() //Collection<Student>
				.stream() //Stream<Stydent>
				.sorted((s1,s2) -> ((Double)s1.getGpa()).compareTo(s2.getGpa()));
	}
	//add a static method to sort student details from the map , as per DoB , ret sorted strm
	static Stream<Student> sortStudentsDoB(Map<String, Student> map)
	{
		return map.values() //Collection<Student>
				.stream() //Stream<Stydent>
